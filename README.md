...
**Testing discovery at scale.** `npm run audit` runs the real company search for a built-in list of about two dozen companies (big tech, startups, conglomerates, companies whose mail domain differs from their website) through the running app and prints one line per company: recruiters found, email domain and where it came from, RocketReach format count and top format, or the stage that failed and why. Pass your own names (`npm run audit -- Stripe "Scale AI"`), a file (`--file companies.txt`, one per line), `--fresh` to bypass the ten-minute result cache, and `--delay 45` to space companies out (default 30 s; RocketReach rate-limits page reads). Nothing is sent and no contacts are saved. The full report lands in `.local-data/audits/` (git-ignored). Each new company costs about 3–4 search-API credits. Delivery itself can only be proven by a run, which is why a run probes one recruiter before emailing the rest.
