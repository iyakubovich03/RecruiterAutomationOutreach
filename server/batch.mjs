...
export function outreachBlocked(history, id, to, multipleCandidates = false) {
 return history.some(h => (h.to === to && ['sent','pending','uncertain'].includes(h.status)) || (h.contactId === id && (['pending','uncertain'].includes(h.status) || (!multipleCandidates && h.status === 'sent'))));
}
