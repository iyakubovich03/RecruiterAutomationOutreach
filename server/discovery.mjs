...
export function companyAssociation(row, company, config = searchConfig) {
  const escaped = compact(company).replace(/[.*+?^${}()|[\]\\]/g,'\\$&').replace(/\s+/g,'\\s+');
  const end = '(?=$|[^\\p{L}\\p{N}])';
  ...
  if (headlineCompany.test(headline) || atCompany.test(headline) || companyRole.test(headline)) return { basis:'LinkedIn search title', text:headline };
  if (atCompany.test(snippet) || roleDotCompany.test(snippet) || companyRole.test(snippet) || labeledCompany.test(snippet)) return { basis:'LinkedIn search snippet', text:snippet };
  return null;
}
...
export async function discoverRecruiters(company, domain='', fetcher=fetch, onEvent=()=>{}, {env={},config=searchConfig}={}) {
  ...
  const query=`site:linkedin.com/in/ "${safeCompany}" ${config.search.roleKeyword}`;
  let rows=[];
  const search=await searchWeb(query,{env,fetcher,read:url=>readPublic(url,fetcher),onEvent,stage:'Recruiter search',accept:results=>{
    ...
    rows=parsed.map(row=>rankProfile(row,company,config)).filter(Boolean);
    onEvent({stage:'Profile filter',status:rows.length?'ok':'empty',detail:`${parsed.length} unique LinkedIn profile links; ${rows.length} matched company, name, and recruiting filters; ${parsed.length-rows.length} excluded`});
  ...
  return {company,domain,provider:search.provider||'Public search',searchedAt:new Date().toISOString(),warnings:search.warnings,results:rows.map(row=>({...row,candidates:domain?candidates(row.name,domain,row.sourceText):[]}))};
}
